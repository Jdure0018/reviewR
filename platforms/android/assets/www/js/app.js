...
var app = {
    ...
    saveReview: function () {
        ...
        // localStorage.getItem('ReviewR-dure0018');
    ...
    cameraSuccess: function (imageURI) {
        ...
        if (myPic) {
            ...
            pictureBtn.style.display = "none"; // picture needs to replace the button
            rateDiv.appendChild(myPic);
            // rateDiv.replaceChild(myPic);
        }else {
            app.showReviews();
            rateDiv.removeChild(myPic);
    ...
    showPictureBtn: function () {
        ...
        var currentPic = document.querySelector('img');
        if (currentPic){
            currentPic.parentNode.childNodes(currentPic);
        }
    }
...
// picture needs to replace the button
//Fix issue where if app closes localStorage items are gone -- issue is known